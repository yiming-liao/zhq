 ...
 * @param precomputeVectors - 是否預先計算每個文件的 TF-IDF 向量，預設 false。
 * @returns 一個物件，包含：
 ...
 */
export function buildIndex<T = unknown>(
  ...
  let docItemsVectors: DocItemsVectors | undefined;
  if (precomputeVectors) {
    docItemsVectors = docItemsTokens.map((tokens) =>
